...
class UserNotificationClient {
   ...
   */
  async notifyPaymentFailed(userId: number, orderNumber: string, reason?: string): Promise<void> {
    const reasonText = reason ? ` Motivo: ${reason}` : '';
    await this.createNotification({
      user_id: String(userId),
      type: 'payment',
      title: 'Error en el pago',
      message: `No se pudo procesar el pago para el pedido ${orderNumber}.${reasonText} Por favor, intenta de nuevo.`,
      action_url: `/dashboard/usuario?tab=orders`
    });
  }
