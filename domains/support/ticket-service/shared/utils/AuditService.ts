...
export class AuditService {
  ...
  async logAuditEntry(
    ...
    try {
      ...
      return {
        ...auditEntry,
        details: auditEntry.details || undefined
      } as AuditLogEntry;
    } finally {
      client.release();
    }
  ...
  async getTicketAuditSummary(ticketId: number): Promise<AuditSummary> {
    ...
    try {
      ...
      const resolvedEntry = auditEntries.find(e => 
        e.action_type === AuditActionType.RESOLVED || 
        (e.action_type === AuditActionType.STATUS_CHANGED && e.new_value === 'resolved')
      );
