    ...
    public recognizeIntent(text: string): SimpleFallbackIntent {
        ...
        for (const [intent, keywords] of this.intentPatterns) {
            ...
            for (const keyword of keywords) {
                // Primero buscar en el texto completo (para frases multi-palabra)
                if (textLower.includes(keyword)) {
                    exactMatches++;
                    continue;
                }

                // Luego buscar en tokens individuales
            ...
            let confidence = totalMatches > 0
                ? Math.min(0.5 + (totalMatches * 0.3), 1.0) // Base 0.5 + bonus por matches
                : 0;

            // Boost para intenciones más específicas
            // Si detectamos keywords de comparación, dar prioridad sobre búsqueda genérica
            if (intent === 'price_comparison' && confidence > 0) {
                confidence = Math.min(confidence + 0.35, 1.0);
            }
            // Penalizar product_search si es muy genérico
