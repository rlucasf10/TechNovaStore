...
import { createLogger, logRequest, logError, logSecurity } from '@technovastore/shared-config';
...
export function createLoggingMiddleware(options: LoggingMiddlewareOptions) {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isSuspicious) {
      logSecurity(logger, 'suspicious_request', {
        method: req.method,
        url: originalUrl,
    ...
    res.end = function(chunk?: any, encoding?: any, cb?: () => void) {
      ...
      if (res.statusCode >= 400) {
        ...
        logger[errorLevel]('HTTP Error Response', {
          ...logData,
